	...
	public class Triangle
	{
		...
		unsafe void LoadTriangleIntoBuffer()
		{
			...
			glVertexAttribPointer(0, 3, GL_FLOAT, false, sizeof(Vertex),
				Marshal.OffsetOf(typeof(Vertex), nameof(Vertex.position)));
			glVertexAttribPointer(1, 4, GL_FLOAT, false, sizeof(Vertex),
				Marshal.OffsetOf(typeof(Vertex), nameof(Vertex.color)));
		...
		{
			this.material.Use();
			this.material.SeTransform(this.Transform.Matrix);
			glBindVertexArray(vertexArray);
